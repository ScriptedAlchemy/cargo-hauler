...
import {
  ...
  statusInputSchema,
  ticketInputSchema,
} from '../../../src/internal/contracts/tool-schemas.js';

/**
 * A settled row exactly as the ledger's row mapper serializes it
 * (`toRequestRecord` in src/internal/storage/ledger.ts): every column present, the
...
/** One window as `metricsWindows` (src/internal/storage/ledger.ts) returns it; the broker adds the id. */
...
describe('await wait ceiling (issues #3, #32)', () => {
  it('bounds one await at the daemon ceiling on the MCP and CLI routes alike', () => {
    ...
    // daemon's wire ceiling is the only bound on one call; `tests/unit/contracts/await-budget.test.ts`
