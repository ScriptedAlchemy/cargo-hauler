 ...
 * `await (await agent()).provider('haulerDaemon')` for every generated MCP
 * tool, routed CLI command, and rendered script. Active daemon observations
 * belong to the operation that needs them, so the root layout performs no
 * socket I/O.
 */
