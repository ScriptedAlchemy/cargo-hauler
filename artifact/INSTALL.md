...
```sh
claude plugin uninstall cargo-hauler@cargo-hauler-marketplace --scope user --keep-data
claude plugin marketplace add ./
claude plugin install cargo-hauler@cargo-hauler-marketplace --scope user
```
...
```sh
claude plugin marketplace remove cargo-hauler-marketplace
```
...
`agent-bundle install claude` records a receipt at `~/.claude/agent-bundle/receipts/cargo-hauler.cargo-hauler-marketplace.user.json`
...
```sh
codex plugin marketplace remove cargo-hauler-marketplace
```
...
It safe-copies the bundle to `~/.cursor/plugins/local/cargo-hauler`. Restart Cursor or run
...
It stages a committed Git repository at `~/.cursor/agent-bundle/marketplaces/cargo-hauler` whose
